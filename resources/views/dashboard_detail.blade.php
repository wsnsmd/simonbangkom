    ...
    <div>
        ...
        <div class="card mt-5">
            <header class="card-header noborder">
                <h3
                    class="font-medium text-lg text-black font-Inter dark:text-white text-center mb-5 lg:mb-0 lg:text-left">
                    {{ $pd->lokasi }}</h3>
                {{-- Excel Button start--}}
                <div class="justify-center sm:justify-end flex  gap-3 items-center flex-wrap">
                    <a class="shift-Away btn btn-sm inline-flex justify-center btn-success rounded-[25px] items-center !p-2.5"
                        data-tippy-content="Export Excel" data-tippy-theme="dark" href="javascript:;"
                        onclick="event.preventDefault(); document.getElementById('export').submit();">
                                        ...
                                        <tr class="border border-slate-100 even:bg-slate-50 dark:border-slate-900 relative">
                                            ...
                                            <td class="table-td">
                                                @if($item['jenis_asn'] == 'PNS')
                                                    @if($item['total_jp'] >= 20)
                                                        <span
                                                            class="badge bg-primary-500 text-white capitalize">{{ $item['total_jp'] }}</span>
                                                    @else
                                                        <span
                                                            class="badge bg-danger-500 text-white capitalize">{{ $item['total_jp'] }}</span>
                                                    @endif
                                                @else
                                                    @if($item['total_jp'] >= 20)
                                                        <span
                                            ...
                                            <td class="table-td">
                                                <div class="action-btns space-x-2 flex">
                                                    <a class="action-btn"
                                                        href="{{ route('dashboard.pegawai', $item['nip_baru']) }}"
                                                        data-tippy-content="Detail" data-tippy-theme="dark">
